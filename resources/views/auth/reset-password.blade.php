    ...
    <form method="POST" action="{{ theme()->getPageUrl('password.update') }}" class="form w-100" novalidate="novalidate" id="kt_new_password_form">
        ...
        <div class="text-center mb-10">
            ...
            <h1 class="text-dark mb-3">
                {{ __('Обновите свой пароль') }}
            </h1>
            <!--end::Title-->

            <!--begin::Link-->
            <div class="text-gray-400 fw-bold fs-4">
                {{ __('Введите новый пароль в поля ниже') }}
            </div>
            <!--end::Link-->
        </div>
        <!--begin::Heading-->
        ...
        <div class="mb-10 fv-row" data-kt-password-meter="true">
            ...
            <div class="passNotDone" data-field="password" data-validator="callback " style="font-size: 0.925rem;color: #F1416C; display: none">Пароли не совпадают</div>
        ...
        <!--begin::Actions-->
        <div class="position-relative mb-3">
            <button type="submit" id="kt_new_password_submit" disabled class="btn btn-lg btn-primary fw-bolder me-4">
                @include('partials.general._button-indicator')
            </button>

            <a href="{{ theme()->getPageUrl('login') }}" class="btn btn-lg btn-light-primary fw-bolder">{{ __('Отменить') }}</a>
        </div>
        <!--end::Actions-->
    </form>
        ...
        <script>
            $( "input[name='password_confirmation']" ).keyup(function() {
                if($("input[name='password_confirmation']").val() == $("input[name='password']").val()){
                    $('.passNotDone').hide();
                    if($("input[name='password_confirmation']").val().match(/[a-z]/i) == null || !$("input[name='password_confirmation']").val().match(/\d/g)){
                        $('.passNotDone').show();
                        $('#kt_new_password_submit').prop('disabled', true);
                        $('.passNotDone').text('Пароль должен содержать хотя бы одну цифру/букву');
                    }else{
                        if ($("input[name='password_confirmation']").val().length <=8){
                            $('.passNotDone').text('Пароль должен содержать 8 и более символов.');
                            $('.passNotDone').show();
                            $('#kt_new_password_submit').prop('disabled', true);
                        }else{
                            $('#kt_new_password_submit').removeAttr("disabled");
                        }
                    }
                }else{
                    $('#kt_new_password_submit').prop('disabled', true);
                    $('.passNotDone').show();
                    $('.passNotDone').text('Пароли не совпадают');
                }
            });
