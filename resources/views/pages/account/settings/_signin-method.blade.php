...
<div class="card {{ $class ?? '' }}" {{ util()->putHtmlAttributes(array('id' => $id ?? '')) }}>
    ...
    <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse" data-bs-target="#kt_account_signin_method">
        <div class="card-title m-0">
            <h3 class="fw-bolder m-0">{{ __('Авторизационные данные') }}</h3>
        </div>
    </div>
    <!--end::Card header-->
        ...
        <div class="card-body border-top p-9">
            ...
            <div class="d-flex flex-wrap align-items-center">
                ...
                <div id="kt_signin_email">
                    <div class="fs-6 fw-bolder mb-1">{{ __('Email адрес') }}</div>
                    <div class="fw-bold text-gray-600">{{ isset(auth()->user()->email) ? auth()->user()->email : '' }}</div>
                </div>
                <!--end::Label-->
                ...
                <div id="kt_signin_email_edit" class="flex-row-fluid d-none">
                    ...
                    <form id="kt_signin_change_email" class="form" novalidate="novalidate" method="POST" action="{{ route('settings.changeEmail') }}">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="current_email" value="{{ isset(auth()->user()->email) ? auth()->user()->email : '' }} "/>
                        <div class="row mb-6">
                            <div class="col-lg-6 mb-4 mb-lg-0">
                                <div class="fv-row mb-0">
                                    <label for="email" class="form-label fs-6 fw-bolder mb-3">{{ __('Введите новый адрес почты') }}</label>
                                    <input type="email" class="form-control form-control-lg form-control-solid" placeholder="{{ __('Почта') }}" name="email" value="{{ old('email') }}" id="email"/>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="fv-row mb-0">
                                    <label for="current_password" class="form-label fs-6 fw-bolder mb-3">{{ __('Введите текущий пароль') }}</label>
                        ...
                        <div class="d-flex">
                            <button id="kt_signin_submit" type="button" class="btn btn-primary  me-2 px-6">
                                @include('partials.general._button-indicator', ['label' => __('Обновить почту')])
                            </button>
                            <button id="kt_signin_cancel" type="button" class="btn btn-color-gray-400 btn-active-light-primary px-6">{{ __('Отмена') }}</button>
                        </div>
                    </form>
                    <!--end::Form-->
                </div>
                ...
                <div id="kt_signin_password_edit" class="flex-row-fluid d-none">
                    ...
                    <form id="kt_signin_change_password" class="form" novalidate="novalidate" method="POST" action="{{ route('settings.changePassword') }}">
                        ...
                        <div class="row mb-1">
                            <div class="col-lg-4">
                                <div class="fv-row mb-0">
                                    <label for="current_password" class="form-label fs-6 fw-bolder mb-3">{{ __('Текущий пароль') }}</label>
                                    <input type="password" class="form-control form-control-lg form-control-solid" name="current_password" id="current_password"/>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-4">
                                <div class="fv-row mb-0">
                                    <label for="password_confirmation" class="form-label fs-6 fw-bolder mb-3">{{ __('Повторите новый пароль') }}</label>
                        ...
                        <div class="form-text mb-5">{{ __('Пароль должен состоять не менее чем из 8 символов и содержать буквы') }}</div>

                        <div class="d-flex">
                            <button id="kt_password_submit" type="button" class="btn btn-primary me-2 px-6">
                                @include('partials.general._button-indicator', ['label' => __('Обновить пароль')])
                            </button>
