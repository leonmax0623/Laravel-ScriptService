<title>{{ __('Регионы') }}</title>
@section('PageName', 'Регионы')
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item text-gray-500">Регионы</li>
@endsection
<x-base-layout>
    ...
    <div class="card" bis_skin_checked="1">
        <div class="card-header border-0 pt-6" bis_skin_checked="1">
            <form method="GET" action="{{route('directory.regionSearch')}}" style="margin: 0 !important;">
                <div class="card-title" bis_skin_checked="1">
                    <div class="d-flex align-items-center position-relative my-1" bis_skin_checked="1">
                        ...
                        <input type="text" name="regionName" class="form-control form-control-sm form-control-solid w-250px ps-14" placeholder="Название региона" value="<?= (isset($_GET['regionName']) && $_GET['regionName']) ? $_GET['regionName'] : '' ?>">
            ...
            <div class="card-toolbar" bis_skin_checked="1">
                ...
                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base" bis_skin_checked="1">
                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal_add_region">
                        Добавить регион
                    </button>
                </div>
                ...
                <div class="modal fade" id="kt_modal_add_region" tabindex="-1" aria-hidden="true" bis_skin_checked="1">
                    <div class="modal-dialog modal-dialog-centered mw-650px" bis_skin_checked="1">
                        <div class="modal-content" bis_skin_checked="1">
                            <div class="modal-header" id="kt_modal_add_region_header" bis_skin_checked="1">
                                <h2 class="fw-bolder">Добавить регион</h2>
                                <div class="btn btn-icon btn-sm btn-active-icon-primary btnCloseModal" data-kt-region-modal-action="close" data-bs-dismiss="modal" bis_skin_checked="1">
                            ...
                            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7" bis_skin_checked="1">
                                <form id="kt_modal_add_region_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" method="POST" action="{{ route('directory.getRegions') }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_region_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_region_header" data-kt-scroll-wrappers="#kt_modal_add_region_scroll" data-kt-scroll-offset="300px" bis_skin_checked="1" style="max-height: 520px;">
                                    ...
                                    <div class="text-center pt-15" bis_skin_checked="1">
                                        <button type="reset" class="btn btn-light me-3" data-kt-region-modal-action="cancel">Сбросить</button>
                                        <button type="submit" class="btn btn-primary" data-kt-region-modal-action="submit">
        ...
        <div class="card-body pt-0" bis_skin_checked="1">
            <div id="kt_table_region_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer" bis_skin_checked="1"><div class="table-responsive" bis_skin_checked="1">
                    <table class="table align-middle table-row-dashed fs-6 gy-3 dataTable no-footer" id="kt_table_region">
                        <thead>
                        <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                            <th tabindex="0" aria-controls="kt_table_region" rowspan="1" colspan="1">Регион, Край, Область</th>
                            <th tabindex="0" aria-controls="kt_table_region" rowspan="1" colspan="1">КФ стратификации</th>
                        ...
                        @foreach ($regions as $region)
                            <tr class="even">
                                <td class="text-gray-800" style="max-width: 200px;">{{ $region->name }}</td>
                                <td class="text-gray-800">{{ $region->temperature_stratification }}</td>
                                <td class="float-end">
                                    <button class="btn btn-primary btn-sm editRegion"
                                            data-bs-toggle="modal"
                                            data-bs-target="#kt_modal_add_region"
                                            user_city_id="{{isset($region->user_city_id) ? $region->user_city_id : 0}}"
                                    ...
                                    <?php if (isset($region->user_city_id) && $region->user_city_id && isset($region->city_id) && $region->city_id == 0) {?>
                                        <a href="/account/directory/deleteRegionData/{{$region->user_city_id}}" class="btn btn-danger btn-sm">
                ...
                <div class="row" bis_skin_checked="1">
                    ...
                    <div class="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end" bis_skin_checked="1">
                        <div class="dataTables_paginate paging_simple_numbers" id="kt_table_region_paginate" bis_skin_checked="1">
    ...
    <script>
        $(document).ready(function () {
            $('[data-bs-target="#kt_modal_add_region"]').on('click', function (){
                $("input[name='name']").val('');
                $("input[name='temperature_stratification']").val('');
                $("input[name='city_id']").val('');
                $("input[name='user_city_id']").val('');
                $('#kt_modal_add_region_header h2').text('Добавить регион');
                $('#kt_modal_add_region_form button span').text('Добавить');
            });
            ...
            $('.editRegion').on('click', function (){
                if ($(this).attr('city_id') != "") {
                    ...
                    $('#kt_modal_add_region_header h2').text('Редактировать регион');
                    $('#kt_modal_add_region_form button span').text('Редактировать');
                }
            });
